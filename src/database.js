...
function createChatConversation({
  ...
  return withTransaction((db) => {
    ...
    const created = db.prepare(
      `
      INSERT INTO chat_conversation (title, created_by_user_id)
      VALUES (?, ?)
      RETURNING id
    `,
    ).run(String(title || "").trim() || null, createdByUserId);
  ...
  return getDb()
    .prepare(
      ...
      INNER JOIN (
        ...
        FROM chat_conversation_participant
        GROUP BY conversation_id
      ) pc ON pc.conversation_id = c.id AND pc.participant_count = 2
      LEFT JOIN LATERAL (
